...
unique_rain_block = df[df['rain_event'].astype(bool)].groupby('rain_block')['rain_event'].nunique()

#print(unique_rain_block)


# map to original df

df['unique_rain_block'] = df['rain_block'].map(unique_rain_block)
...
# Fixed File Path
df2.to_csv('C:\\Users\\ResononScanningSyst\\Desktop\\AutoMills\lesion_result.csv')
